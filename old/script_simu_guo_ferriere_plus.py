...
from clouds_old import Cloud_sphere, Cloud_ring, Cloud_ellipsoid, Cloud_cyllinder
...
def calculate_local_density_1cloud(X,Y,Z, cloud):
    ...
    if cloud.rot_matrix == None:
        ...
        for idx, valx in enumerate(X): # iter over several arrays, mutli index (éventuellement)
            for idy, valy in enumerate(valx):
                for idz, valz in enumerate(valy):

                    x = Xp[idx, idy, idz].value
                    y = Yp[idx, idy, idz].value
                    z = Zp[idx, idy, idz].value

                    vec = np.array([x, y, z]) # Xp.ravel pour les trois matrices
                    vec_rotated = cloud.rot_matrix.apply(vec) # essayer de voir si on peut vectorisé cette étape
...
def run_sim(n_atlas, spacebins, min_time, max_time, time_bins, min_energy, max_energy, energy_bins):
    ...
    for t in times:
        ...
        map_2D = compute_map(cloud_atlases[n_atlas], 
                    nbins_3d=spacebins, 
